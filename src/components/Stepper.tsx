...
export default function Stepper({
  ...
  return (
    <div className="outer-container" {...rest}>
      <div className={`step-circle-container border border-zinc-800 ${stepCircleContainerClassName}`}>
        ...
        {!isCompleted && (
          <div className={`footer-container ${footerClassName}`}>
            <div className={`footer-nav ${currentStep !== 1 ? 'spread' : 'end'}`}>
              {currentStep !== 1 && (
                <button
                  type="button" // default button type should be button
                  onClick={handleBack}
    ...
    <motion.div onClick={handleClick} className="step-indicator" style={disableStepIndicators ? { pointerEvents: 'none', opacity: 0.5 } : {}} animate={status} initial={false}>
      <motion.div
        variants={{
          ...
          active: { scale: 1, backgroundColor: '#4ade80', color: '#000' },
          complete: { scale: 1, backgroundColor: '#4ade80', color: '#000' }
        }}
...
function StepConnector({ isComplete }: StepConnectorProps) {
  const lineVariants: Variants = {
    incomplete: { width: 0, backgroundColor: 'rgba(74,222,128,0)' },
    complete: { width: '100%', backgroundColor: '#4ade80' }
  };
